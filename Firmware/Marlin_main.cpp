  ...
  update_current_firmware_version_to_eeprom();
    ...
    if (eeprom_read_byte((uint8_t*)EEPROM_UVLO) == 1) { //previous print was terminated by UVLO
      ...
      if ( degBed() > ( (float)eeprom_read_byte((uint8_t*)EEPROM_UVLO_TARGET_BED) - AUTOMATIC_UVLO_BED_TEMP_OFFSET) ){ 
        ...
        #endif 
        if ( lcd_show_fullscreen_message_yes_no_and_wait_P(MSG_RECOVER_PRINT, false) ){
          recover_print(0); 
        } else { 
...
//Not used on MKPRO
#ifdef PAT9125 //Not defined for us
  void fsensor_init() {
...
int serial_read_stream() {
  ...
  uint32_t receivedBytes = 0;
    ...
    while (prusa_sd_card_upload) {
      for (int i=0, int data; i<CHUNK_SIZE; i++) {
...
void process_commands() {
  ...
  if (code_seen("M117")) { //moved to highest priority place to be able to to print strings which includes "G", "PRUSA" and "^"
    ...
    */
  ...
  } /*else if (code_seen("FAN")) { //Disabled as fan control is moved to slave board -PD
    MYSERIAL.print("E0:");
    ...
    #else
      MYSERIAL.println("SD NOT SUPPORTED\n");
    #endif
    ...
    switch((int)code_value()) {
      ...
      case 1: // G1
        if(Stopped == false) {
          //Some motherfu*3r destroyed the function here for filament runout sensor support
          //need to add that back in at some point...
          ...
          if (total_filament_used > ((current_position[E_AXIS] - destination[E_AXIS]) * 100)) { //protection against total_filament_used overflow
            ...
            if(autoretract_enabled){
              if( !(code_seen('X') || code_seen('Y') || code_seen('Z')) && code_seen('E')) { //If only E change requested
	...
	*/
	...
	case_G80:
	{
		...
		while (mesh_point != MESH_MEAS_NUM_X_POINTS * MESH_MEAS_NUM_Y_POINTS) {
			...
			if (iy & 1){
        ix = (MESH_MEAS_NUM_X_POINTS - 1) - ix; // Zig zag
      }
      float z0 = 0.f;
...
//REMOVED AS WE ONLY RUN LOUD AF BABY
...
void update_currents() {
  //Do nothin now bish cause we loud af
}
...
void get_coordinates() {
  ...
  for(int8_t i=0; i < NUM_AXIS; i++) {
    if(code_seen(axis_codes[i])) { //If request is for one of the movable axis
...
//Function to move all axis to their designated targets
//input: none
//output: none
//I personally would prefer if none of this were done with global variables. Rather IMO 
//it should be variables passed to this function as the destination axis
void prepare_move() {
